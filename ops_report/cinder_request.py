...
class CinderClient(object):
    ...
    def __init__(self, token, cinder_ip=None, port=None, project_id=None):
    ...
    def pools_stats(self):
        if self.port is None:
            full_url_cinder = '{0}://{1}/v2/' \
                            '{2}/scheduler-stats/get_pools?detail=True'.format(common.ssl,self.cinder_ip,self.project_id)
        else:
            full_url_cinder = '{0}://{1}:{2}/v2/' \
                            '{3}/scheduler-stats/get_pools?detail=True'.format(common.ssl,self.cinder_ip,
    ...
    def pools_stats_detail(self):
       ...
       # print(list_pools)
        ...
        for pool in list_pools:
            ...
            output[key_name_pool].update({'total_mb': pool.get('capabilities').get(
                'total_capacity_gb')})
            output[key_name_pool].update({'used_mb': (pool.get('capabilities').get(
